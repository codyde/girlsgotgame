  ...
  try {
    ...
    const children = await db.query.user.findMany({
      ...
      columns: {
        ...
        jerseyNumber: true,
        role: true,
        accountType: true,        // NEW: Distinguish registered vs manual players
        hasLoginAccess: true,     // NEW: Whether they can log in
        createdAt: true,
      }
    });
...
// Admin: Get parent-child relations (legacy endpoint for compatibility)
router.get('/admin/relations', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.error('Get parent-child relations error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
